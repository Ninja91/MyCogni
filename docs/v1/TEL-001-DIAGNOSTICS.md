...
- `EventId` and `EVENT_CATALOG` bind each event to exact required and optional fields plus an
  explicit component/action/result/level combination matrix. Exception category/result and
  connector ID/version pairs are checked separately.
...
- Job, action, and trace correlations require distinct factory-only `DiagnosticJobId`,
  `DiagnosticActionId`, and `DiagnosticTraceId` values. They have no parse/from-domain factory, so
  a generic/profile `OpaqueId`, UUID string, encoded UUID, or a different correlation purpose
  fails exact runtime type checks. External identifiers, profile IDs, identity values, and stable
  cross-installation identifiers are prohibited.
...
- Durations, retry numbers, counts, field cardinality, and the final JSON line are bounded.
  Booleans and coercible strings do not count as integers.
- `classify_exception` uses only built-in exception type relationships and returns a finite
  category, including explicit `asyncio.CancelledError` handling. It never calls `str`, `repr`,
  traceback formatting, or exception argument access.
- `LocalJsonSink` writes canonical newline-delimited JSON to an injected local text stream. It has
  no network, exporter, logging-framework, telemetry, or auto-instrumentation dependency and
  raises when the stream reports a short write.
...
- Purpose-specific IDs prevent accidental generic/profile/cross-purpose reuse; they do not prove a
  caller has not deliberately encoded sensitive information elsewhere or bypassed the type API.
  Opaque UUIDs also remain locally correlatable when repeated. Retention must remain short and
  access-controlled.
