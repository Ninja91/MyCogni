...
entrypoint warns and confirms before consuming that offline route. Rotated reprovision is executable from the
...
non-TTY refusal, generic reprovision rejection, destructive-confirmation refusal, two copied-string-only
reprovision rotations, exact replacement-root redaction, and empty actual stdout/stderr. Later recovery and
reprovision inputs are parsed from completed operator handoffs, not reconstructed from caller-supplied
bindings. The artifact contains no retained deterministic credential value.
