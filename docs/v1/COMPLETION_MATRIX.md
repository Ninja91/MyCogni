...
| Full traceability validator | GOV-001 | `IN_PROGRESS` | machine registries and adversarial fixtures plus two `reviews/08-gov001-v4-adversarial-review.md` REJECT cycles | isolate configured trust roots from branch ancestry, reject vacuous HEAD recovery and close/narrow remaining structural-witness constants; independent re-review required |
...
| Network-deny proof | NET-001 | `IN_PROGRESS` | `docs/v1/NET-001-NETWORK-DENY.md`; exact source/node provenance, revocable task/thread/descriptor authority, launcher-exclusion and complete patch-integrity regressions, 84 focused and 886 full-suite lane tests | independently re-review remediation and reproduce merged dual-Python evidence; OS-level hostile-code containment remains a nonclaim |
