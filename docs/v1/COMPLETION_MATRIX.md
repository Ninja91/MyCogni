...
| Auth/key/egress/runner/browser/backup spikes | SPIKE-* | `IN_PROGRESS` | auth `030caed`; runner `b2bfa15`; auth and runner review records; ADR-0013, `spikes/SPIKE-KEY.md`, `reviews/16-spike-key-design-adversarial-review.md`, `reviews/17-spike-key-exact-target-adversarial-review.md`; auth/runner have final code-level ACCEPT, while key targets `2a144bf`/`b74afdb`/`4f6f0ca`/`211c9ee`/`a0ae32a`/`89baaa3` were rejected and the remediation candidate passes 106 focused tests | authenticated package acceptance, durable production adapters and runner-specific OCI containment remain open; key needs clean exact review and host conformance; egress/browser/backup remain open |
