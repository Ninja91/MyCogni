...
| Synthetic simulator/network-deny harness | `IN_PROGRESS` | SIM-001 is integrated; NET-001 adds shell-accurate fail-closed test options, exact normalized node/AST/code/module callable binding, revocable task/thread/descriptor authority, 100 focused and 902 full-suite lane tests | independent NET-001 re-review and merged dual-runtime reproduction; optional Linux namespace reproduction remains open |
...
| Network-deny proof | NET-001 | `IN_PROGRESS` | `e09c720`, `docs/v1/NET-001-NETWORK-DENY.md`, `reviews/09-net001-adversarial-review.md`; quoted/malformed `PYTEST_ADDOPTS` and collector/callable identity remediated with 100 focused, 87 simulator and 902 dual-runtime lane tests | independently re-review latest remediation and reproduce merged dual-Python evidence; OS-level hostile-code containment remains a nonclaim |
| Auth/key/egress/runner/browser/backup spikes | SPIKE-* | `IN_PROGRESS` | `1931d20`, `docs/v1/spikes/SPIKE-AUTH.md`; volatile auth ceremony has 84 focused and 949 dual-runtime lane tests; `reviews/10-m0-next-package-readiness.md` makes runner mailbox/protocol handoff-ready | three-hat SPIKE-AUTH review is pending; remaining spikes are not started; final runner OCI/browser disposition requires PF-002 real-engine evidence |
