...
| Synthetic simulator/network-deny harness | `IN_PROGRESS` | SIM-001 is integrated; NET-001 adds exact node/AST/code/module authority, revocable task/thread/descriptor leases and a pre-import launcher that rejects plugin/config overrides; 133 focused and 935 dual-runtime lane tests | independent final NET-001 re-review and merged dual-runtime reproduction; optional Linux namespace reproduction remains open |
...
| Network-deny proof | NET-001 | `IN_PROGRESS` | `362795e`, `docs/v1/NET-001-NETWORK-DENY.md`, `reviews/09-net001-adversarial-review.md`; plugin/config injection is rejected before import, locked plugins are explicit, 133 focused/87 simulator/935 dual-runtime lane tests | independently re-review latest remediation and reproduce merged dual-Python evidence; OS-level hostile-code containment remains a nonclaim |
