...
| Auth/key/egress/runner/browser/backup spikes | SPIKE-* | `IN_PROGRESS` | auth `030caed`; runner `b2bfa15`; auth and runner review records; ADR-0013, `spikes/SPIKE-KEY.md`, `reviews/16-spike-key-design-adversarial-review.md`, `reviews/17-spike-key-exact-target-adversarial-review.md`; auth/runner have final code-level ACCEPT, while the remediated key replacement candidate passes 59 focused tests and awaits repeat exact review | authenticated package acceptance, durable production adapters and runner-specific OCI containment remain open; key host conformance plus egress/browser/backup spikes remain open |
