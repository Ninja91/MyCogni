...
| Canonical inventory: SPIKE-BROWSER | SPIKE-BROWSER | `IN_PROGRESS` | ADR-0015, `spikes/SPIKE-BROWSER.md`, `SPIKE-BROWSER-OCI-EVIDENCE.md`, exact source/Compose/runtime verifiers and safe mutation tests; native-arm64 Docker Desktop synthetic-only evidence | independent adversarial review, exact SIGTERM/reaping, shared-mount disposition, amd64/native-Linux matrix, minimal signed/SBOM artifact and gateway/connector work remain open; `BROW-001` stays `NOT_STARTED` |
