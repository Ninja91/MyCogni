...
| Interactive architecture walkthrough | `IN_PROGRESS` | static `site/`, pinned Pages workflow, matrix/deployment-linked offline guard and five mutation tests; two P1 remediation rounds implemented | independent static re-review, browser-backed keyboard/responsive/WCAG inspection, remote links and actual Pages publication remain open |
...
| Synthetic simulator/network-deny harness | `IN_PROGRESS` | SIM-001 is integrated; NET-001 remediation adds a guarded pytest launcher, 38-node source-bound numeric-loopback registry, revocable task/thread/descriptor authority, finite PII-safe diagnostics and merged 912-test dual-runtime evidence | fix quoted environment-option composition and collector/callable identity; independent NET-001 re-review; optional Linux namespace reproduction remains open |
...
| Network-deny proof | NET-001 | `IN_PROGRESS` | `docs/v1/NET-001-NETWORK-DENY.md`, `reviews/09-net001-adversarial-review.md`; most original findings fixed and merged dual-Python 912-test evidence | fix quoted `PYTEST_ADDOPTS` exclusion composition and exact collector/callable marker identity, then re-review; OS-level hostile-code containment remains a nonclaim |
| Auth/key/egress/runner/browser/backup spikes | SPIKE-* | `NOT_STARTED` | `reviews/10-m0-next-package-readiness.md` makes SPIKE-AUTH and the pure SPIKE-RUNNER mailbox/protocol slice handoff-ready | start after GOV/NET remediation; final runner OCI disposition and browser spike require PF-002 real-engine evidence |
