...
| Canonical inventory: SPIKE-BROWSER | SPIKE-BROWSER | `IN_PROGRESS` | exact source `d656c08`, native-arm64 image `sha256:6e443f187b62...`; ADR-0015, `spikes/SPIKE-BROWSER.md`, `SPIKE-BROWSER-OCI-EVIDENCE.md`, exact source/Compose/runtime verifiers and safe mutation tests; three exact Docker Desktop runtime-verifier passes including root-orchestrator reproduction | independent adversarial review, exact SIGTERM/reaping, shared-mount disposition, amd64/native-Linux matrix, minimal signed/SBOM artifact and gateway/connector work remain open; `BROW-001` stays `NOT_STARTED` |
