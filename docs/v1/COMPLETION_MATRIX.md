...
| Canonical inventory: SPIKE-RUNNER | SPIKE-RUNNER | `IN_PROGRESS` | `b2bfa15`, `reviews/13-spike-runner-adversarial-review.md`, ADR-0014, `RUNNER-PERSISTENCE-OCI-EVIDENCE.md`; final source acceptance plus successor canonical SQLite restart and trusted-sidecar Docker evidence | authenticated package acceptance, separate untrusted connector artifact/gateway topology, restore/backup recovery, signed artifact and full malicious-runtime evidence remain open; PF-002 and the sidecar smoke do not prove connector acceptance |
