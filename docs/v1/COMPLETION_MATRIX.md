...
| Interactive architecture walkthrough | `IN_PROGRESS` | `86c7f17`, `reviews/11-site-adversarial-review.md`; source/offline ACCEPT with zero P0/P1/P2, browser-backed keyboard/responsive reproduction, pinned Pages workflow, matrix/deployment-linked guard and nine mutation tests on both runtimes | screen-reader/full WCAG inspection, remote links and actual Pages publication remain open |
...
| Auth/key/egress/runner/browser/backup spikes | SPIKE-* | `IN_PROGRESS` | `ff428c1`, `docs/v1/spikes/SPIKE-AUTH.md`, `reviews/12-spike-auth-adversarial-review.md`; first auth remediation passed 40 focused and 952 dual-runtime lane tests but product/operator and backend re-reviews still REJECT | bind grants to successful store-recorded step-up provenance; complete initial recovery handoff and repeatable reprovision; bound replay/root/GC semantics; then complete three-hat re-review |
