...
| Local KEK provider | one explicit provider/reference with no fallback or runtime provisioning; strict A256GCM wrapped-profile-key/AAD v1; owner-only native file baseline outside every managed root | native macOS/Linux permissions, restart/fork/sentinel/recovery tests; separate macOS Security.framework, rootless Linux Engine and Docker Desktop conformance rows or named blockers | M0 |
